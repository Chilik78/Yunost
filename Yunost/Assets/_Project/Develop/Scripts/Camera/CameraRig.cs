...
namespace CameraModule
{
    public class CameraRig : MonoBehaviour
    {
        ...
        private void FollowPlayer()
        {
            ...
            transform.position = Vector3.SmoothDamp(transform.position, targetCoord, ref _vel, smoothTime); // ѕлавно перемещает камеру в точку координату персонажа 
        }
        ...
        void SwitchToTransparent(GameObject gameObject)
        {
            if (!_hiddenGameObjects.ContainsKey(gameObject))
            {
                ...
                {
                    Debug.LogWarning($"Ќе удалось сделать прозрачным {gameObject.name}");
                }
            }
